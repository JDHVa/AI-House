...
class VisionLogic:
    ...
    def detect_faces(self, frame):
        ...
        try:
            # Reducción para velocidad (como en el segundo código)
            ...
            for enc in encs:
                ...
                if True in matches:
                    ...
                    verified = True
                    print(f"[DEBUG] Usuario verificado: {name}")
...
class App:
    ...
    def start_login_loop(self, e):
        ...
        for intento in range(max_intentos):
            ...
            if verified and len(names) > 0:
                ...
                # Dibujar en frame original (MEJORADO: recuadro que sigue a la cara)
                for (top, right, bottom, left), n in zip(locs, names):
                    # Escalar coordenadas de vuelta al tamaño original (x4 porque redimensionamos a 0.25)
                    top = int(top * 2) 
                    right = int(right * 2)
                    bottom = int(bottom * 2)
                    left = int(left * 2)
                    ...
                    # Dibujar rectángulo principal (como en el segundo código)
                    cv2.rectangle(frame, (left, top), (right, bottom), color, 2)
                    
                    # Dibujar rectángulo para el nombre (como en el segundo código)
                    cv2.rectangle(frame, (left, bottom - 35), (right, bottom), color, cv2.FILLED)
                    
                    # Texto del nombre (como en el segundo código)
                    cv2.putText(frame, n, (left + 6, bottom - 6), 
    ...
    def capture_loop(self):
        ...
        while self.running:
            ...
            # 1. Detección de rostros (SIEMPRE, como en el segundo código)
            locs, names, verified = self.logic.detect_faces(frame)
            
            # 2. Registrar (Si se pulsó botón en Flet) - COMO EN EL SEGUNDO CÓDIGO
            if self.save_req and self.inp_name.value and self.mode == "REGISTRO":
                ...
                if nombre:
                    ...
                    # Usar el método del segundo código para registrar
                    ok, msg = self.logic.register_new_face(save_frame, nombre)
            ...
            # 3. Dibujar en el video (COMO EN EL SEGUNDO CÓDIGO - RECUADRO QUE SIGUE)
            for (top, right, bottom, left), name in zip(locs, names):
                # Escalar coordenadas (x4 porque redimensionamos a 0.25 en detect_faces)
                top = int(top * 4)
                right = int(right * 4)
                bottom = int(bottom * 4)
                left = int(left * 4)
            ...
            # 4. Actualizar Estado en Flet (como en el segundo código)
            if not self.save_req and len(names) > 0:
                if verified:
                    status_text = f"✅ HOLA: {names[0]}"
                else:
            ...
            if self.mode == "GESTOS":
                ...
                # Actualizar estado (combinando gestos y rostros)
                if current_time - last_gesture_time > 0.5:
                    if len(names) > 0:
                        face_status = f"✅ {names[0]}" if verified else f"⛔ {names[0]}"
                        status_text = f"🎭 {gesture_text} | 👤 {face_status} | 📊 {self.fps:.1f} FPS"
                    else:
                        status_text = f"🎭 {gesture_text} | 👤 No detectado | 📊 {self.fps:.1f} FPS"
                    self.safe_update_status(status_text)
                ...
                cv2.putText(frame, f"Usuario: {self.user_name}", (10, 60), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                cv2.putText(frame, f"Modo: GESTOS", (10, 90), 
                ...
                if len(names) > 0:
                    cv2.putText(frame, f"Rostro detectado: {names[0]}", (10, 170), 
